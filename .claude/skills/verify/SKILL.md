...
Useful DOM handles: `#popup` (entity inspector popup inside `#map`), `#pops` (population labels), `#events` (event feed with tick numbers), `#timescale button:text-is("64x")` (speed buttons: pause/1x/8x/64x), `#forge-btn` (buy the next pick tier), `#recap` (the welcome-back toast).

## Pick upgrades (the forge button)

Colony pick tiers live in `data/upgrades.toml` (a NEW REQUIRED data file: a data dir missing it fails to boot; refresh every scratch/fixture dir from `data/`). Each `[[upgrade]]` has a `name`, `cost` (colony gold), and `damage`. Tiers are bought in order with shared colony gold; the ws snapshot ships the whole table as `upgrades` plus the current `upgradeLevel` (also on every tick message). `#forge-btn` shows the next tier and its cost (e.g. "Copper Picks (3g)"), is disabled unless you have a living dwarf and enough gold, and reads "picks maxed" once every tier is owned. Buying sends `{type:"upgrade", player}` over ws; the server deducts the cost and bumps `upgradeLevel`. `upgradeLevel` boosts EVERY miner's damage by the summed `damage` of purchased tiers (`World.MineBonus`), so a dwarf's per-tick mine damage is `mine_damage + bonus`. Ground truth for the boost is the ws `mining` map: at 1x each tick message is one sim tick, so a cell's damage climbs by `mine_damage + bonus` per message (verified: +1/tick at level 0, +2/tick after the first tier whose `damage` is 1, so mold's 6 hit points break in 3 ticks instead of 6). Higher timescales batch N sim ticks per broadcast, so sample deltas at 1x. UpgradeLevel persists in the save and starts at 0 for an existing world.
