...
`data/upgrades.toml` is now a level CURVE plus a random draw POOL (a NEW REQUIRED data file: a data dir missing it fails to boot; refresh every scratch/fixture dir from `data/`). Two scalars define the curve: `level_base` (mined gold for the first level) and `level_growth` (each level costs `level_growth`x the last, rounded up). Each `[[upgrade]]` is a pool entry: `name`, `kind` (`damage`, `luck`, `weapon`, `beam`, or `missile`), `amount`, `max` (0 = unlimited), and weapons/beams/missiles add `color`/`radius`/`period_ms`. Cumulative gold EARNED AT THE MARKET (`GoldMined`, lifetime, never spent) drives levels: dwarves with `carry_capacity` (dwarf: 3) mine ORE into their bag (`ore` on the EntityView, event type "ore"), walk it to the market structure (entity type with `market = true`, spawned by worldgen near the campfire and auto-spawned into old saves on load), and deposit (`action` "hauling ore" then "selling", event type "sold" with `amount`); Gold and GoldMined move ONLY at the deposit, so the level bar jumps at the chest, not the rock face. Types without carry_capacity keep the old instant-gold path. Claimed `speed` upgrades (Swift Boots, +25%/claim) multiply every walk via `World.SpeedFactor`. On the canvas the market renders as the 0x72 golden chest, loaded dwarves carry a 3px `#d9c14a` nugget, and a gold `+N` float pops at the seller on deposit; the ws snapshot and every tick ship `level`, `goldMined`, `prevLevelGold`, `nextLevelGold`, `pendingLevels` (unclaimed level count), `offer` (the current up-to-3 choices), `claims` (name->count), and the whole `upgrades` table. `prevLevelGold`/`nextLevelGold` ride the wire so the client never hardcodes the curve. Each level reached with any eligible entry (`max` 0 or claims below `max`) increments `pendingLevels` and fires a "level" event; the server keeps ONE offer rolled at a time (up to 3 distinct eligible names, world-RNG deterministic). Legacy saves with the old `pending` name queue migrate on load: each queued name becomes one pending level.
...
The economy is a torch-driven loop: mold only forms in DARK passable cells, and a dwarf can only mine LIT mineable faces, so each torch lights a finite mold pocket (a handful of ORE at mold's 10% gold_chance) that the dwarf mines out, hauling bagfuls to the market, and then idles. Warning for e2e: WHICH face a fresh dwarf picks is nearest-face geometry; it can lock onto a day-scale rock face beside the mold, and mine targets persist until mined out or unlit. Boost damage via debug claims to make any face breakable when the test needs deposits. Also note pathfinding is light-blind: a shortest path through unlit cells triggers the flee-the-dark loop (a known, pre-existing quirk). Sustained leveling means re-torching freshly-spread dark mold; to fast-forward, place torches on dense rock-free mold pockets then `POST /api/advance`.
