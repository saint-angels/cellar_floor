...
Beams are the first NON-AOE damage: a claimed `beam` upgrade (Lance, white `#ffffff`, amount 3) adds `World.BeamBonus` damage ONLY to the miner's chosen target face (`mt` on the EntityView), not to the other swept neighbors, so the target's per-tick delta runs `mine_damage + MineBonus + BeamBonus` while neighbors get `mine_damage + MineBonus`. Client-side the beam is a thin line (radius = length in px) that aims parked behind the dwarf's back for 55% of `period_ms`, flies through the target face until 80%, spawns debris and a shake on arrival, then vanishes until the next cycle.
